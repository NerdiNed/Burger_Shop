...
const fs = require('fs')
// creating a function from the adminRoute to be able to pass a parameter because i want the meals file to only be used from one source ! 
...
function adminPageRouter(myMeals) {
        ...
        if (mealTitle && mealPrice && mealDescription && req.files) {
            ...
            if (findMatchMeal) {
                ...
                mealImg.mv('./public/uploadedfiles/' + mealTitle.replace(/ /g, '_') + myMeals.length + ext).then(() => {
                    ...
                    // normally i would save it in a database or for simulation create an own json file for it. but for now i will keep it here and push it into the meals array
    ...
    adminRoute.post('/deletemeal', (req, res) => { //*1
        ...
        // deleting a meal from the meals array
    ...
    adminRoute.post('/editmeal', (req, res) => {
        ...
        //should only run if there is an image . so adding an if-statement
        if (req.files) {
            console.log(req.files);
...
//*3
// because i wrote middleware to secure all the routes i commented this for showing purpose